<table width="100%" border="0" cellspacing="0" cellpadding="0" class="viewer">
  ...
  if (isset($_SESSION['produk_itemproduk'])) {
        foreach ($_SESSION['produk_itemproduk'] as $key => $val){
  ...
  <tr>
    ...
    <td align="center">
		<a href="produk_cart.php?act=plus&amp;barang_id=<?php echo $key; ?>&amp;ref=index.php?page=produk_permintaan_material_produksi2">+</a> |
		<a href="produk_cart.php?act=min&amp;barang_id=<?php echo $key; ?>&amp;ref=index.php?page=produk_permintaan_material_produksi2">-</a> |
		<a href="produk_cart.php?act=del&amp;barang_id=<?php echo $key; ?>&amp;ref=index.php?page=produk_permintaan_material_produksi2">Hapus</a>
	</td>
  </tr>
  <?php
  ...
  <tr>
	<td align="right" colspan="2"><strong>TOTAL :
		<?php
			$total = 0;
			foreach ($_SESSION['produk_itemproduk'] as $key2 => $val){
  ...
  <tr>
    ...
    <td align="center" colspan="2"><br/>
		<form action="" method="POST">
			...
			<a class="btn btn-danger btn-sm" href="produk_cart.php?act=clear&amp;ref=index.php?page=produk_permintaan_material_produksi2">Batal</a>
		</form>
	</td>
  </tr>
</table>
...
<?php
	if(isset($_POST['simpan'])){
		...
		$query2 = mysql_query ("INSERT INTO permintaan_produksi
								VALUES ('".$id_permintaan."', '".$today."', 'P1' ,'".$_SESSION['admin']."' , ".$total.", ".$max.", 'pending') ");
		
		if ($query2 == 1)
		{
			echo '<script languange="javascript">alert ("Permintaan Berhasil di Kirim")</script>';
			...
			if (isset($_SESSION['produk_itemproduk'])) {
				foreach ($_SESSION['produk_itemproduk'] as $key => $val) {
					unset($_SESSION['produk_itemproduk'][$key]);
				}
				unset($_SESSION['produk_itemproduk']);
			}
		} else {
			echo '<script languange="javascript">alert ("Permintaan Gagal di Proses")</script>';
		}
		echo '<script languange="javascript">window.location="index.php?page=produk_permintaan_material_produksi2"</script>';
	}
?>